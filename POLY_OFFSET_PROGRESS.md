...
- [x] **Scan-stripe regression bisected to Apr 5 commit `bb1f465`** which replaced `compute_offsets_guess` (per-frame, per-chunk mean from raw detector FITS — 98.5% nonzero x0 init) with `compute_x0_from_Ab` (diagonal-LS from the LSQR A matrix — only ~4% nonzero on narrow channel masks like D3 Ch17). With `iter_lim=50`, the under-constrained inactive chunks leak per-frame DC into scan-stripe residuals at mosaic time. Localized by 3-way mosaic of `cal_*_damp0p1_*.h5` (March production, clean) / `cal_*_baseline_after_commit3.h5` (multi-chunk-maps PR state — striped) / `cal_*_baseline_after_poly_off.h5` (current branch — striped). Re-running cal with `compute_offsets_guess` x0 init reproduces the production result; LSQR arnorm drops from 9.7 (under-converged) to 0.20 (converged).
- [x] **Fix: explicit per-frame scalar column.** Added `use_per_frame_scalar=False` to `setup_lsqr` (and `Calibrator.setup_lsqr`) that adds an explicit per-frame scalar block, decoupled from the legacy `det_groups_list` path. Added `compute_x0_scalar_only(A, b, ref_shape, scalar_col_start)` x0 helper: scalar gets `compute_x0_from_Ab`'s diag-LS (≈ weighted mean of valid b per frame), chunks + sky start at 0. Combined with `mean_offsets_list=[zeros]` to anchor each map's per-frame chunk-mean to 0, this pushes per-frame DC entirely into the scalar so chunks only carry within-frame structure. `run_cal_v2.py` and `run_cal_baseline_test.py` now use this by default. `scalar_off` test variant matches the production mosaic visually and within `σ_clip=9.92e-3` (production 9.84e-3), with LSQR arnorm=0.88 (converged) instead of 9.7 (under-converged).
