...
- [x] **`_prep_subframe` interp_matrix shape fall-through.** Previously when a caller passed `chunk_maps=None` (or `[]`) but a non-None `grid_valid_weight` / `det_aux`, the interp_matrix construction triggered with `interp_input_shape=None` and crashed inside `make_linear_interp_matrix` (`H, W = None` → TypeError). Pre-existing — also broken in the legacy single-map code (`np.shape(None) → ()` → ValueError there). Fix: collect detector-grid shapes from any of `chunk_maps[0]`, `grid_valid_weight`, `det_aux[0]` that are provided, assert they agree (cheap defensive check against silent shape mismatches), and use the consensus. If the interp matrix is needed but no shape source was supplied, raise a `ValueError` with a clear message instead of a `H, W = None` traceback. The trigger condition also dropped its standalone `for_lsqr` clause — `for_lsqr=True` with empty chunk_maps was always a no-op (empty `chunk_contribs` list comp), so it shouldn't have been forcing interp_matrix construction. Verified across five cases (the previously-buggy gvw-only / det_aux-only paths, the legacy K=1 path, shape-mismatch detection, and the no-shape-source error message).
