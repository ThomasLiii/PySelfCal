...
gets the resolved callable. Off by default — the production drivers all left
``postprocess_func=None``. Kept here so a config can opt in by name without
editing Python.
"""
import numpy as np
...
def mask_bright_pixels(local_vars):
    ...
    subframe. Copied verbatim from the legacy driver helper."""
