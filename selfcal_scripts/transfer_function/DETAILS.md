...
- `[mosaic]` mean map only (`make_std_map = false`,
  `apply_sigma_clipping = false`, `cache_intermediate = false`, no wavelength
  coaddition): the transfer function is read off `MEAN_MAP`, and the other
  maps cost roughly half the mosaic's wall time without being read. This is a
  mosaic-side choice only — the calibration recipe above is the fiducial one,
  and `MEAN_MAP` is the same map whether or not the others are built, to within
  the coadd's run-to-run non-determinism (it accumulates in worker-completion
  order, so no coadd configuration is bit-reproducible).
