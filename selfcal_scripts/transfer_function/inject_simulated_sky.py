#!/usr/bin/env python3
"""Inject a simulated sky into real reprojected frames, for a transfer-function run.

Workflow: to measure the selfcal pipeline's transfer function you replace the
real sky in each reprojected frame with a KNOWN simulated sky, run the fiducial
calibration + mosaic, and compare the output mosaic to the sky you put in.
...
simulated sky, leaving EVERYTHING ELSE untouched (``sub_mapping``, ``sub_bitmask``,
...
injection is a plain rectangular crop::
    ...
    sub_data_new = simulated_sky[y0:y1, x0:x1]
...
Requirements on the simulated sky:
...
Example
-------
    python inject_simulated_sky.py \
        --frames-in  /mnt/md124/.../SPHEREx_..._D3_.../reprojected \
        --frames-out /scratch/tf/D3_simsky_frames \
        --simulated-sky /scratch/tf/sim_sky_D3.npy \
...
def load_simulated_sky(path, ref_shape):
    """Load the simulated sky (.npy or .fits) and check it matches the ref grid."""
    ...
    if sky.shape != ref_shape:
        raise ValueError(
            f"simulated sky shape {sky.shape} != reference grid shape {ref_shape}. "
            "The simulated sky must be on the SAME grid the frames were reprojected "
            "onto (the detector's ref.fits image shape).")
    return sky


def _inject_one(args):
    src, dst, sim_sky_path, ref_shape, preserve_footprint = args
    # Load the simulated sky per-worker (cheap vs. the h5 IO; keeps the pool picklable).
    sky = load_simulated_sky(sim_sky_path, ref_shape)
    ...
    with h5py.File(dst, "r+") as f:
        ...
        if crop.shape != orig.shape:
            raise ValueError(
                ...
                f"{orig.shape} (ref_coords={[y0, y1, x0, x1]}). The simulated sky "
...
def main():
    ...
    # Validate the simulated sky up front (fail fast before copying 900 files).
    load_simulated_sky(args.simulated_sky, ref_shape)
    ...
    print(f"Injecting simulated sky into {len(src_files)} frames "
          f"({args.frames_in} -> {args.frames_out}) with {args.workers} workers...")

    jobs = [(s, os.path.join(args.frames_out, os.path.basename(s)),
             args.simulated_sky, ref_shape, not args.no_preserve_footprint)
            for s in src_files]
    ...
    print(f"Done. {len(jobs)} simulated-sky frames written to {args.frames_out}")
