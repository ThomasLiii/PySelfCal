...
| [`repair_anchor.py`](repair_anchor.py) | Phase-1 repair. Flags channels with `pearson_r < --r-threshold` (default 0.5; catches airglow blowouts like D1 He I Ch31-33 while leaving D4 PAH at r≈0.85-0.92 alone). For flagged channels it **smooths only the slope** (Pearson-r-weighted spline fit to the **clean channels only**, so a blown channel can't leak in) and then **recomputes C** as `mean_full_dc - slope_final*mean_pred` — C is NOT smoothed, so the non-zodi signal (He glow, DGL) it carries is preserved; only the mis-calibrated zodi part is corrected. Clean channels keep their raw fit. Updates the anchor file **in-place**; raw `slope`/`intercept` are preserved. `--dry-run`/`--plot` to inspect first; `--s-factor` tunes slope smoothing. Core: `SelfCal.ZodiAnchor.rweighted_spline_repair`. |
