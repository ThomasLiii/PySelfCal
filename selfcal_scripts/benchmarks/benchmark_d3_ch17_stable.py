"""Benchmark cal + mosaic for D3 Ch17 on the *stable* branch (pre-multi-chunk-maps).

Self-contained: inlines a PhaseTracker (mirroring the one in main's benchmark
scripts) and the prepare_*_inputs helpers (since stable doesn't have
run_cal_baseline_test.py).
...
API permits:
  ...
  - LSQR iter_lim=50, atol/btol=1e-6, precondition=True, solver='lsqr',
    use_float32=True, n_threads=32
  - mosaic oversample=2, std+sigma_clip=True, sigma=2.0
  - cache_batch_size=20, coadd_batch_size=30
  - wav_coadd batch_size=40, max_workers=30

Differences from main (forced by API):
  - mean_offsets=None  (stable's old code used compute_offsets_guess for x0;
    we use compute_x0_from_Ab with num_offset_groups=num_frames since both
    paths share the LSQR matrix; this matches main's pre-scalar workflow)
  - no use_per_frame_scalar (option doesn't exist on stable)
...
class PhaseTracker:
    ...
    def plot_timeline(self, path):
        ...
        dr_rate = smooth(dr_rate); dw_rate = smooth(dw_rate)
        ...
        fig.suptitle('Pipeline benchmark (STABLE) — D3 Ch17, NumCol=3, no poly', fontsize=11)
        fig.savefig(path, bbox_inches='tight')
        plt.close(fig)


# ============================================================
# Inlined detector/channel input helpers (stable single-form API)
# ============================================================
...
def prepare_channel_inputs(ch, frame_setting, det_chunk_map, grid_chunk_map):
    ...
    if isinstance(ch, (list, np.ndarray)):
        ...
        chunk_valid_mask_padded = make_stripped_chunk_valid_mask(
            subch=subch, num_subchannels=num_subchannels, num_channels=num_channels,
            num_columns=num_columns, subchannel_padding=1,
        )
        chunk_valid_mask = make_stripped_chunk_valid_mask(
            subch=subch, num_subchannels=num_subchannels, num_channels=num_channels,
...
def main():
    ...
    tracker.start()
    print(f"[bench:stable] config: D{frame_setting['Detector']} ch={chs[0]} NumCol={frame_setting['NumCol']} no_poly single-form-API", flush=True)
    ...
    with tracker.phase('transfer_hdd_to_nvme'):
        ...
        if n_existing == len(hdd_reproj_files):
            print(f"[bench:stable] NVMe cache already populated ({len(hdd_reproj_files)} files); skipping copy", flush=True)
        else:
            print(f"[bench:stable] transferring {len(hdd_reproj_files) - n_existing} files HDD->NVMe...", flush=True)
    ...
    with tracker.phase('cal_setup_lsqr'):
        cc.setup_lsqr(
            chunk_map=det_inputs['det_chunk_map'],                  # single-form
            grid_valid_weight=ch_inputs['det_valid_mask_padded'],
            oversample_factor=1,
            adj_info=det_inputs['adj_info'],                         # single-form
            **calibration_kwargs,                                    # reg_weight (single)
        )

    with tracker.phase('cal_warmstart'):
        x0 = compute_x0_from_Ab(cc.A, cc.b, cc.ref_shape, num_frames_run)
    ...
    try:
        ...
        with tracker.phase('mosaic_make_mosaic_total'):
            maps = mm.make_mosaic(
                chunk_map=det_inputs['grid_chunk_map'],                # single-form
                grid_valid_weight=ch_inputs['grid_valid_weight'],
                oversample_factor=mosaic_oversample_factor,
                det_offset_func=partial_make_offset_map,                # single-form
                cache_dir=cache_dir,
                **mosaic_kwargs,
            )
    finally:
    ...
    txt_path = os.path.join(BENCH_DIR, 'd3_ch17_stable_summary.txt')
    json_path = os.path.join(BENCH_DIR, 'd3_ch17_stable_samples.json')
    png_path = os.path.join(BENCH_DIR, 'd3_ch17_stable_timeline.png')
    ...
    print(f"\n[bench:stable] wrote {txt_path}")
    print(f"[bench:stable] wrote {json_path}")
    print(f"[bench:stable] wrote {png_path}")
