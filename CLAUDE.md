...
| `selfcal_scripts/spectral_4pass/` | **Superseded** by the `npass` runner task (`configs/sep_d4_npass.toml` is the same recipe at `n = 4`); kept only until the migration's SEP/NEP runs finish, then deleted. Its `data/pah_template.npz` (the SEP aromatic template, BC grid to 3.47 µm) moves to `selfcal/instruments/spherex/data/line_templates/`. Modes `pahfit_subch`/`pahfit_lvf`/`pahfit_lvf_polybasis` live in `runner/modes/pahfit_lvf.py`. |
