...
| `selfcal_scripts/zodi_anchor/` | Post-hoc zodi anchor. **Non-mutating sidecar architecture**: `build_*` (zodipy predictions → `zodi_preds/*.npz`), `build_sidecar.py` (per-channel fit → `<run>/zodi_anchor/anchor_D{N}.h5`, leaves cal/mosaic pristine), `diag_*` (read-only plots; apply the anchor in-memory via `load_anchor`), `revert_anchor.py` (one-shot undo of legacy in-place anchoring). `apply_*` / `pipeline_multi_channel.py` are DEPRECATED in-place mutators. See [`selfcal_scripts/zodi_anchor/README.md`](selfcal_scripts/zodi_anchor/README.md). Core math + sidecar I/O + consumer in `SelfCal/ZodiAnchor.py` (`fit_anchor_for_channel`, `write_sidecar`, `Anchor`/`load_anchor`). |
