...
| `selfcal_scripts/drivers/` | Production drivers: `run_reproject.py`, `run_cal.py`, `diff_cal_h5.py` (schema-aware cal-file diff), `precompute_lvf_params.py`. |
...
| `selfcal_scripts/experiments/` | Non-mainline drivers: `run_cal_k2_readout.py` (D5 K=2 readout-stripe variant), `run_mosaic_polyconstraint.py` (poly-constraint mosaic variant). |
...
Pipeline runs are normally launched via `selfcal_scripts/drivers/run_cal.py` (calibration + mosaic) and `selfcal_scripts/drivers/run_reproject.py` (reprojection). They import `SelfCal.PipelineWrapper` (resolved via `pip install -e .` — no `sys.path` manipulation needed). Edit the `frame_setting` / config block at the top, then run with `python selfcal_scripts/drivers/run_cal.py`. They pin `OMP/MKL/OPENBLAS_NUM_THREADS=1` so the in-process LSQR threadpool is the only parallelism — preserve that when adding launchers.
