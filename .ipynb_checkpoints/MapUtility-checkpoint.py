...
def bit_to_bool(bitmask_array, ignore_list, bitmask_header, invert=False):
    # This is a static utility, could also live in a general utils module
...
def make_weight(frame, sigma=1.4):
    ...
    convolved_frame = gaussian_filter(filled_frame, sigma=1.4) - frame
...
def det_to_sub(det_mapping=None, n_chunk=10, sub_width = 157, det_size = 2040, det_off=None):
    det_grid_x, det_grid_y = det_mapping
    upgrade_factor = int(np.shape(det_grid_x)[-1]/sub_width)
    ...
    chunk_contrib = coo_matrix((data, (rows, cols)), shape=(n_chunk**2, (sub_width*upgrade_factor)**2))
    chunk_contrib_bin = bin2d_coo_matrix(chunk_contrib, sub_width*upgrade_factor, sub_width*upgrade_factor, upgrade_factor)
...
def compute_chunk_contrib(det_mapping, chunk_map, sub_width, det_width, upgrade_factor):
    num_chunks = len(np.unique(chunk_map))
    pix_idx = det_mapping.astype(np.int32)
    valid_x = (pix_idx[0] >= 0) & (pix_idx[0] <= det_width-1)
    valid_y = (pix_idx[1] >= 0) & (pix_idx[1] <= det_width-1)
    valid_pix = valid_x & valid_y
    chunk_idx = np.full(pix_idx.shape[1:], fill_value=-1, dtype=chunk_map.dtype)
    chunk_idx[valid_pix] = chunk_map[pix_idx[1][valid_pix], pix_idx[0][valid_pix]] # Index 1 of det_mapping is y_grid and 0 is x_grid

    valid_idx_flat = np.nonzero(valid_pix.ravel())[0]
    chunk_idx_flat = chunk_idx.ravel()[valid_idx_flat]

    rows = chunk_idx_flat
    cols = valid_idx_flat
    data = np.ones_like(rows, dtype=np.float32)
    map_width = sub_width * upgrade_factor
    chunk_idx_parsed = coo_matrix((data, (rows, cols)), shape=(num_chunks, map_width * map_width))
    chunk_contrib = bin2d_coo_matrix(chunk_idx_parsed, map_width, map_width, upgrade_factor)
